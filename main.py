...
def main():
    ...
    try:
        if is_bitlink(token, args.url):
            ...
            click_counter = count_clicks(token, args.url)
            print('Количество кликов по ссылке {url} = {count}'.format(
                url=args.url,
                count=click_counter))
        else:
            print('Вы ввели URL, создаю битлинк...')
            bitlink = shorten_link(token, args.url)
            print('Битлинк для адреса {url}: {short_link}'.format(
                url=args.url,
                short_link=bitlink))
    except requests.exceptions.RequestException() as err:
        print('Error, check input link. Error message: {error}'.format(
            error=err))
